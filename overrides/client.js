  ...
  async chatCompletion({ payload, abortController = null }) {
    ...
    try {
      ...
      const runAgent = async (agent, _messages, i = 0, contentData = [], _currentIndexCountMap) => {
        ...
        function llamaSafeZod(s) {
          const t = s?._def?.typeName;
          if (!t) return s;

          if (t === 'ZodNullable') return llamaSafeZod(s._def.innerType).optional();
          if (t === 'ZodOptional') return z.optional(llamaSafeZod(s._def.innerType));

          if (t === 'ZodObject') {
            const shape = s.shape; // s.shape is a getter in recent Zod
            const newShape = Object.fromEntries(
              Object.entries(shape).map(([k, v]) => [k, llamaSafeZod(v)]),
            );
            return z.object(newShape).passthrough(); // <-- critical: avoids ZodNever
          }

          if (t === 'ZodArray') return z.array(llamaSafeZod(s._def.type));
          if (t === 'ZodRecord') return z.record(llamaSafeZod(s._def.valueType));
          if (t === 'ZodUnion') return z.union(s._def.options.map(llamaSafeZod));
          if (t === 'ZodDiscriminatedUnion')
            return z.union([...s._def.optionsMap.values()].map(llamaSafeZod));
          if (t === 'ZodTuple') return z.tuple(s._def.items.map(llamaSafeZod));
          if (t === 'ZodDefault')
            return llamaSafeZod(s._def.innerType).default(s._def.defaultValue());
          if (t === 'ZodEffects')
            return z.ZodEffects.create(llamaSafeZod(s._def.schema), s._def.effect);
          return s;
        }
        if (agent.tools) {
          for (let i = 0; i < agent.tools.length; i++) {
            agent['tools'][i].schema = llamaSafeZod(agent['tools'][i].schema);
          }
        }
        //agent['tools'][0].schema = llamaSafeZod(agent['tools'][0].schema)
        //agent['tools'][1].schema = llamaSafeZod(agent['tools'][1].schema)
        //console.log(agent['tools'][0].schema)
        // schema = llamaSafeZod(schema);
